...
test('hiding official BMD listings preserves third-party resources and shared preferences',()=>{
  ...
  assert.equal(filterEntries(data.entries,{...DEFAULTS}).length,448);
  assert.equal(DEFAULTS.sort,'name');
});
test('website contains all 448 unique catalogue entries and no private outreach data',()=>{assert.equal(data.entries.length,448);assert.equal(new Set(data.entries.map(e=>e.id)).size,448);assert.equal(new Set(data.entries.map(e=>e.url)).size,448);const json=JSON.stringify(data);assert.doesNotMatch(json,/C:\\\\Users|D:\\\\Projects|gmail_draft_id|contact_email|gmail-receipts/);});
