...
function details(e){openDialog(e.name);const b=$('#dialog-body');b.append(make('p',e.description),link('Open original resource ↗',e.url),facts(e));if(e.evidence.length)b.append(make('h3','Evidence and limitations'),evidenceList(e));if(e.history.length)b.append(make('h3','Version history'));if(e.history.length)for(const h of e.history){const p=make('p',h.from+' → '+h.to+' · '+h.summary+' ');p.append(link('Source',h.changelogs?.[0]?.url||h.source));b.append(p);b.append(changelogView(h));}const issue=new URL('https://github.com/subtlesayak/subtle-resolve-list/issues/new');issue.searchParams.set('template','creator-confirmation.yml');issue.searchParams.set('title','Confirm or correct: '+e.name);b.append(link('Confirm or correct this listing ↗',issue.href));}
