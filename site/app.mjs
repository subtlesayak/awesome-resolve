import {DEFAULTS,UNFILTERED,TASKS,LEVELS,filterEntries,sortEntries,relativeDate,versionText,stateFromUrl,stateToUrl,facetCounts,adaptTaskSelection,recoveryOptions} from './model.mjs?v=7';
...
function details(e){openDialog(e.name);const b=$('#dialog-body');b.append(make('p',e.description),link('Open original resource ↗',e.url),facts(e),make('h3','Evidence and limitations'),evidenceList(e));if(e.unknownFields.length)b.append(make('p','Still unknown: '+e.unknownFields.join(', ')+'.','muted'));b.append(make('h3','Version history'));if(e.history.length)for(const h of e.history){const p=make('p',h.from+' → '+h.to+' · '+h.summary+' ');p.append(link('Source',h.changelogs?.[0]?.url||h.source));b.append(p);if(h.changes?.length)b.append(changelogView(h));}else b.append(make('p','No before-and-after version change has been recorded yet. The current observation is shown above.'));const issue=new URL('https://github.com/subtlesayak/awesome-resolve/issues/new');issue.searchParams.set('template','creator-confirmation.yml');issue.searchParams.set('title','Confirm or correct: '+e.name);b.append(link('Confirm or correct this listing ↗',issue.href));}
