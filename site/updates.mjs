...
export function releaseNoteBlocks(text) {
 ...
 for(const line of text.split(/\r?\n/)) {
  ...
  } else blocks.push({type:heading?'heading':'paragraph',text:heading?heading[1]:line});
 }
 return blocks;
}
function releaseNotes(text, entries) {
 const root=make('div',undefined,'release-notes');
 for(const block of releaseNoteBlocks(text)) {
  if(block.type!=='list') {
   const el=make(block.type==='heading'?'h3':'p');appendInline(el,inlineTokens(block.text));root.append(el);continue;
  }
  for(const group of creatorGroups(block.items,entries,line=>line.match(/\]\((https:\/\/[^)]+)\)/)?.[1])) {
   if(group.creator && group.items.length>1)root.append(make('h4',group.creator,'creator-heading'));
   const list=make('ul');
