...
async function fetchPage(url){
 ...
 try{
  const response=await fetch(url,{signal:AbortSignal.timeout(25000),headers:{'User-Agent':'Awesome-Resolve-Link-Check/1.0','Accept':'text/html,application/json;q=0.9,*/*;q=0.5'}});
  let raw=''; const reader=response.body?.getReader();let size=0;
  if(reader){for(;;){const {done,value}=await reader.read();if(done)break;size+=value.length;if(size>6000000){await reader.cancel();break;}raw+=new TextDecoder().decode(value);}}
  ...
  for(const m of raw.matchAll(/<a\b([^>]*?)>([\s\S]*?)<\/a>/gi)){
   ...
   try{const u=new URL(decode(href),response.url);if(u.protocol==='https:'&&!u.search)links.push({url:u.href,label:label.slice(0,120)});}catch{}
  }
  const snippets=[];
  ...
  return {url,final_url:response.url,checked_at:started,http_status:response.status,status:blocked?'challenge':response.ok?(text.length<800&&!product?'limited_content':'retrieved'):'http_error',title:textOf(raw.match(/<title[^>]*>([\s\S]*?)<\/title>/i)?.[1]||'').slice(0,180),text_length:text.length,product_metadata:product,update_links:[...new Map(links.map(e=>[e.url,e])).values()].slice(0,12),update_snippets:snippets,page_date_candidates:dates};
