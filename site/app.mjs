import {DEFAULTS,TASKS,LEVELS,filterEntries,sortEntries,relativeDate,versionText,stateFromUrl,stateToUrl,facetCounts,adaptTaskSelection,recoveryOptions} from './model.mjs?v=3';
...
function updateFilterOptions() {
  const omitted = [];
  const taskScope = {...DEFAULTS, task:state.task, official:state.official};
  for(const select of $('#filters').querySelectorAll('select')) {
    ...
    const scope = select.name==='task' ? {...DEFAULTS,official:state.official} : state;
...
function renderRecovery(empty) {
  ...
  for(const option of options) {
    const label=option.key==='taskOnly'?'Keep task, clear other filters':option.key==='clear'?'Clear filters':'Remove '+filterNames[option.key]+' filter';
    actions.append(resultAction(label+' ('+option.count+' resources)',option.patch));
  }
...
function renderFilterSummary() {
  ...
  const active = [...$('#filters').elements].filter(el => el.name && state[el.name]);
  $('#filter-label').textContent = 'Filters and setup' + (active.length ? ' · ' + active.length + ' active' : '');
  group.hidden = !active.length;
  for (const el of active) {
    const label = el.type === 'checkbox' ? 'Official BMD resources' : el.parentElement.firstChild.textContent.trim();
    const value = el.type === 'checkbox' ? 'Hide' : originalOptions.get(el.name)?.find(o=>o.value===state[el.name])?.label || state[el.name];
