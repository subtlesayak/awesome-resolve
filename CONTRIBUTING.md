...
For resource additions or substantive catalogue updates, add a dated entry to `CHANGELOG.md` listing every added resource, its purpose, access conditions, compatibility caveats, and discovery source. Group the update into one GitHub release using those notes; start at `v1.0` and increment the minor version for each release (`v1.1`, `v1.2`, and so on), regardless of the date. README edits, layout fixes, sorting, and other presentation-only changes use ordinary commits and do not need individual releases or version bumps. Tag the verified catalogue commit, and keep unresolved leads clearly labelled. Catalogue releases do not imply upstream software releases or installation testing.
